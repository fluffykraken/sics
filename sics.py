...
def update_progress(iteration, total, length=40):
    """Displays a progress bar in the same line."""
...
if total_points:
    ...
    for i, point in enumerate(points, start=1):
        time_str = point["time"]
        success = False  # Track success for progress updates

        try:
            if ACTION_MODE == "delete":
                query = f'''
                DELETE FROM "{MEASUREMENT}"
                WHERE time = '{time_str}' AND "entity_id"::tag = '{ENTITY_ID}'
                '''
                client.query(query)
                success = True
            else:
                ...
                delete_query = f'''
                DELETE FROM "{MEASUREMENT}"
                WHERE time = '{time_str}' AND "entity_id" = '{ENTITY_ID}'
                '''
            ...
            sys.stdout.write(f"\n❌ Error during {ACTION_MODE} of {time_str}: {e}\n")
            sys.stdout.flush()
            continue

        # ✅ Update progress bar **only if action was successful**
        if success:
            update_progress(i, total_points)

    print(f"\n✅ All values were successfully {'deleted' if ACTION_MODE == 'delete' else 'overwritten'}.")
client.close()
